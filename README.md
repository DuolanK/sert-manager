...
Фронтенд вызывает API по относительному пути `/api/*` (то есть на тот же origin, где
открыт сам сайт), а Next.js через rewrite проксирует эти запросы на контейнер `nginx`
внутри docker-сети. Благодаря этому:

- не нужно прописывать IP/домен сервера в коде или `.env`;
- нет CORS-запросов и блокировки Private Network Access (проблема `localhost` на сервере);
- одна и та же сборка работает и локально, и на удалённом сервере.

Целевой хост прокси задаётся переменной `API_PROXY_TARGET` (по умолчанию `http://nginx`).
